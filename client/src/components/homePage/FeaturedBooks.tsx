...
export function FeaturedBooks() {
  ...
  const fetchFeaturedBooks = async () => {
    const { data } = await api.get(`/featuredbooks`);
    ...
    <section id="books" className="py-16 md:py-24">
      <div className="container mx-auto px-4">
        <div className="text-center space-y-4 mb-16">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">Handpicked reads from the community to spark your next great discussion.</p>
        </div>
            ...
            <div key={index} className="group cursor-pointer">
              <div className="bg-card rounded-2xl p-6 border border-border transition-all duration-300 group-hover:shadow-lg group-hover:border-primary/20">
                <div className="aspect-[3/4] mb-4 rounded-lg overflow-hidden bg-muted">
                  <img src={`${book.coverImg}` || "/placeholder.svg"} alt={`${book.title} — book cover`} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
                </div>
